...
def smart_extract(transcript: str, call_type: str = "demo") -> dict:
    ...
    addr_match = re.search(
        r'\d{3,5}\s+[A-Z][A-Za-z\s]+(?:Road|Rd|Street|St|Avenue|Ave|Blvd|Boulevard|Drive|Dr|Way|Lane|Ln|Parkway|Pkwy|Industrial)[,\s]+[A-Za-z\s]+,\s*[A-Z]{2}\s+\d{5}',
        text
    )
    if addr_match:
        result["office_address"] = addr_match.group(0).strip()
    ...
    # Fail message
    fail_phrases = [
        r"tell.*caller.*(?:we'?re|someone will|help is)",
        r"apologize.*assure",
        r"someone will (?:call|reach|contact)"
    ]
    for fp in fail_phrases:
        m = re.search(fp, text_lower)
        if m:
            start = max(0, m.start() - 20)
            end = min(len(text), m.end() + 100)
            snippet = text[start:end].strip()
            result["call_transfer_rules"]["message_if_fails"] = snippet[:200]
            break
    ...
    if not result["emergency_routing_rules"]["contact_order"]:
...
def run_all():
    ...
    for demo_file in demo_files:
        ...
        with open(out_path / "memo.json", "w") as f:
            json.dump(memo, f, indent=2)
        with open(out_path / "agent_spec.json", "w") as f:
            json.dump(agent_spec, f, indent=2)
        with open(out_path / "meta.json", "w") as f:
            json.dump({"account_id": account_id, "version": "v1", "company": memo.get("company_name"), "generated_at": datetime.utcnow().isoformat() + "Z"}, f, indent=2)
    ...
    for onboard_file in onboard_files:
        ...
        # Load v1
        with open(OUTPUTS_DIR / account_id / "v1" / "memo.json") as f:
            v1_memo = json.load(f)
        ...
        # Merge: onboarding data wins, but don't overwrite with nulls
        merged = v1_memo.copy()
        ...
        merged["version_history"] = {
            ...
            "v2_generated_at": datetime.utcnow().isoformat() + "Z"
        }

        agent_spec_v2 = generate_agent_spec(merged, "v2")
        ...
        with open(v2_path / "memo.json", "w") as f:
            json.dump(merged, f, indent=2)
        with open(v2_path / "agent_spec.json", "w") as f:
            json.dump(agent_spec_v2, f, indent=2)
        with open(v2_path / "meta.json", "w") as f:
            json.dump({"account_id": account_id, "version": "v2", "company": merged.get("company_name"), "generated_at": datetime.utcnow().isoformat() + "Z", "total_changes": changelog["total_changes"]}, f, indent=2)

        # Save changelog
        cl_path = OUTPUTS_DIR / account_id / "changelog"
        cl_path.mkdir(parents=True, exist_ok=True)
        with open(cl_path / "changes.json", "w") as f:
            json.dump(changelog, f, indent=2)
        with open(cl_path / "changes.md", "w") as f:
    ...
    with open(OUTPUTS_DIR / ".." / "batch_report.json", "w") as f:
